...
class DQNTrain(QNTrain):
    ...
    def process_state(self, state):
        ...
        #####################################################################
        state = torch.from_numpy(state/self.config.high).float().to(device=self.device)
    ...
    def forward_loss(
        ...
        #       specified above.
        #####################################################################
        Qs = self.q_net(state).gather(1, action.unsqueeze(-1)).squeeze(-1)
        
        next_Qs = self.target_q_net(next_state).max(1)[0]
        
        
        Q_expectation = reward + self.config.gamma * next_Qs
        
        Q_samp = reward + (1-done_mask)*Q_expectation
        
        loss = torch.mean(torch.pow((Q_samp - Qs),2))
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        #####################################################################
        s_batch, sp_batch = self.process_state(s_batch), self.process_state(sp_batch)
