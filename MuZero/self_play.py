...
class SelfPlay:
    ...
    def continuous_self_play(self, shared_storage, replay_buffer, test_mode=False):
        while (shared_storage.get_info("training_step") < self.config.training_steps and not shared_storage.get_info("terminate")):
            ...
            #visit_softmax_temperature_fn will produce a float that depends on number of training steps.
            # overtime the value is decreased to reduce the likelihood of exploration.
            if (not test_mode):
            ...
            if not test_mode and self.config.ratio:
                while ( shared_storage.get_info(
                    ("training_step") / max( 1, shared_storage.get_info("num_played_steps")) < self.config.ratio)
                    and (shared_storage.get_info("training_step") < self.config.training_steps)
    ...
    def play_game(self, temperature, temperature_threshold, render, opponent, muzero_player):
        ...
        # Context-manager that disabled gradient calculation. Disabling gradient calculation is useful for inference to reduce memory usage
        with torch.no_grad():
            while(not done and len(game_history.action_history) <= self.config.max_moves):
                ...
                # choose action depending on if it's muzero's turn or opponenets
