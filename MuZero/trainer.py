...
class Trainer:
    ...
    def continuous_update_weights(self, replay_buffer, shared_storage):
        ...
        while (self.training_step < self.config.training_steps and not shared_storage.get_info("terminate")):
            ...
            if self.config.ratio:
                while (
                    self.training_step / max(1, shared_storage.get_info("num_played_steps")) > self.config.ratio
    ...
    def update_weights(self, batch):
        ...
        priorities[:, 0] = (
            numpy.abs(pred_value_scalar - target_value_scalar[:, 0])** self.config.PER_alpha
        )
        ...
        for i in range(1, len(predictions)):
            ...
            priorities[:, i] = (
                numpy.abs(pred_value_scalar - target_value_scalar[:, i]) ** self.config.PER_alpha
            )
